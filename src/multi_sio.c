...
u32 MultiSioMain(void *sendp, void *recvp, u32 loadRequest)
{
    ...
    case 0: 
        if (!sioCntBak.id)
        {
            ...
            if (!sioCntBak.si && gMultiSioArea.sendBufCounter == -1)
            {
                ...
                // this one should be MULTI_SIO_TIMER_COUNT, but it's changed in an unknown way
                REG_MULTI_SIO_TIMER = 0xBBBC; // Timer Initialization
    ...
    case 1:
        ...
        {
            if (gMultiSioArea.multisioarea_unk0a <= 7)
                ++gMultiSioArea.multisioarea_unk0a;
            else
    ...
    // the following looks extremely stupid for a real match. 
    r0 =       gMultiSioArea.recvSuccessFlags
    ...
    ret = r0;
    if (gMultiSioArea.multisioarea_unk09)
    {
        r0 = MULTI_SIO_UNK_x1000;
        ret |= r0;
    }
    r1 = gMultiSioArea.multisioarea_unk0a >> 3 << 15;
    if (sioCntBak.id > 3)
    {
        r0 = MULTI_SIO_UNK_x2000;
        r0 |= r1;
        r0 |= ret;
    }
    else
