...
static void sub_080A5188(struct Object *blipper)
{
    ...
    if (blipper->base.yspeed > 0)
    {
        if (blipper->base.x <= gCurLevelInfo[blipper->base.unk56].levelMaxPosition.x
    ...
    if (blipper->base.xspeed > 0)
    {
        if (blipper->base.x + 0x800 <= gCurLevelInfo[blipper->base.unk56].levelMaxPosition.x
...
static void sub_080A5B1C(struct Object *blipper)
{
    ...
    else if (blipper->base.x <= gCurLevelInfo[blipper->base.unk56].levelMaxPosition.x
        && blipper->base.x >= gCurLevelInfo[blipper->base.unk56].levelMinPosition.x
        && blipper->base.y <= gCurLevelInfo[blipper->base.unk56].levelMaxPosition.y
        && blipper->base.y >= gCurLevelInfo[blipper->base.unk56].levelMinPosition.y)
    {
...
static void sub_080A6CBC(struct Object *blipper, u8 a2)
{
    ...
    effect->x = blipper->base.x;
    effect->y = blipper->base.y;
    effect->parent = blipper;
    effect->roomId = blipper->base.roomId;
    effect->flags |= 0x4000;
    switch (a2)
    {
    case 0:
    ...
    if (Macro_0810B1F4(&blipper->base))
        effect->flags |= 0x2000;
    EffectObjectInitSprite(effect, &effect->sprite, 6, 0x329, 2, 0xC);
...
static void sub_080A6E44(void)
{
    ...
    else
    {
        ...
        if (blipper)
        {
            ...
            if (!blipper)
                goto label;
            if (Macro_0810B1F4(&blipper->base) && !(effect->flags & 0x2000))
            {
