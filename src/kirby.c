...
{
    bool32 r4 = FALSE;
    struct Object *obj = kirby->base.unk6C;
    ...
    {
        if (obj
            && (obj->unk83 >= gUnk_08351648[obj->type].unk0
                || obj->base.unkC & 0x40))
        {
            r4 = TRUE;
            kirby->base.x = obj->base.x;
        }
        if (r4)
        {
            obj->base.unkC |= 0x80;
    ...
    if (kirby->base.counter != obj->unk83)
    {
    ...
    if (!(r8[kirby->idleTimer].flags & 0x200))
    {
        ...
        if (kirby->base.flags & 1)
            kirby->base.x = obj->base.x - r8[kirby->idleTimer].unk0 * 0x100;
        else
            kirby->base.x = obj->base.x + r8[kirby->idleTimer].unk0 * 0x100;
        kirby->base.y = obj->base.y + r8[kirby->idleTimer].unk1 * 0x100;
    }
    if (!--kirby->flyTimer)
    {
        ++kirby->idleTimer;
