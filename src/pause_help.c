...
static inline void PauseMenuInit(struct PauseMenu* pauseMenu, u32 playerId, struct Task* task) {
    ...
    if (playerId < gNumHumanPlayers) {
        pauseMenu->flags = MENU_FLAG_PLAYER;
    }
    else {
        pauseMenu->flags = MENU_FLAG_AI;
    }
    if (pauseMenu->playerId == gLocalPlayerId) {
...
void CreatePauseMenu(void) {
    ...
    if (gPauseMenus[gLocalPlayerId].menuId == MENU_AREAMAP) {
        CreateAreaMap();
    }
    else if (gPauseMenus[gLocalPlayerId].menuId == MENU_WORLDMAP) {
...
void CreateHelpMenu(void) {
    ...
    if (gLocalPlayerId == gUnk_0203AD50) {
...
static inline void guard_sub_08031CE4(u8 playerId) {
    if (gUnk_0203AD10 & 2 && playerId != gLocalPlayerId) {
        sub_08031CE4(8);
    }
}
...
static void PauseMenuMain(void) {
    ...
    if (allPressedKeys & SELECT_BUTTON) {
        enum PauseMenuId menuId = gPauseMenus[gLocalPlayerId].menuId;

        if (gPauseMenus[gLocalPlayerId].menuId == MENU_WORLDMAP) {
            menuId = MENU_HELP;
        }
        else if (gPauseMenus[gLocalPlayerId].menuId == MENU_AREAMAP) {
        ...
        if (menuId != gPauseMenus[gLocalPlayerId].menuId) {
            for (playerId = 0; playerId < 4; playerId++) {
    ...
    if (gPauseMenus[gLocalPlayerId].menuId == MENU_AREAMAP && allPressedKeys & (R_BUTTON | L_BUTTON) &&
