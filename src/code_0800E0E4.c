...
static void sub_0800E27C(void)
{
    ...
    else
    {
        ...
        if (!(var->objBase.flags & 0x1000))
        {
                    ...
                    for (i = 0; i < sp08->unk40; ++i)
                    {
                        ...
                        if (sp14 > r5 && gCollisionAttributes[GetCollisionTile(var->objBase.unk56, r5, r6)] & 0x40)
                        {
                            sub_08001408(var->objBase.unk56,
                                ...
                                0, 0);
                            PlaySfx(&var->objBase, SE_BLOCK_BREAK);
            ...
            {
                s16 sp00[] = { (gCurLevelInfo[gCurrentPlayerId].viewportPosition.x >> 8) - 8, (gCurLevelInfo[gCurrentPlayerId].viewportPosition.y >> 8) - 8 };
                struct Sprite *r6;
                ...
                if (!sl)
                {
                    ...
                    for (i = 0; i < preload; ++i)
                    {
                        ...
                        if (!(var->objBase.flags & 0x400)
                            && gKirbys[gCurrentPlayerId].base.base.base.roomId == var->objBase.roomId)
                        {
